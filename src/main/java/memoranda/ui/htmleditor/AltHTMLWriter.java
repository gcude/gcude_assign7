...
public class AltHTMLWriter extends AbstractWriter {
	...
	/**
	 * Iterates over the Element tree and controls the writing out of all the tags
	 * and its attributes.
	 *
	...
	public void write() throws IOException, BadLocationException {
		...
		while ((next = it.next()) != null) {
			...
			if (current != null) {
				...
				if (indentNeedsIncrementing(current, next)) {
					...
					/*
					 * next and current are not siblings so emit end tags for items on the stack
					 * until the item on top of the stack, is the parent of the next.
					 */
					...
					/*
					 * if next and current are siblings the indent level is correct. But, we need to
					 * make sure that if current is on the stack, we pop it off, and put out its end
					 * tag.
					 */
					Element top = (Element) blockElementStack.peek();
					if (top == current) {
		...
		/*
		 * A null parameter ensures that all embedded tags currently in the tags vector
		 * have their corresponding end tags written out.
		 */
		...
		/*
		 * if (completeDoc) { writeAdditionalComments(); }
		 */
	 ...
	 * If the tag isnt found, then the tag is removed from the vector and a
	...
	/**
	 * Determines if the element associated with the attributeset is a TEXTAREA or
	 * SELECT. If true, returns true else false
	 */
	...
	/**
	 * Determines whether a the indentation needs to be incremented. Basically, if
	 * next is a child of current, and next is NOT a synthesized element, the indent
	 * level will be incremented. If there is a parent-child relationship and "next"
	 * is a synthesized element, then its children must be indented. This state is
	 * maintained by the indentNext boolean.
	 *
	...
	/**
	 * Outputs the styles as a single element. Styles are not stored as elements,
	 * but part of the document. For the time being styles are written out as a
	...
	/**
	 * Outputs the named style. <code>outputStyle</code> indicates whether or not a
	 * style has been output yet. This will return true if a style is written.
	 */
	...
	/**
	 * Convert the give set of attributes to be html for the purpose of writing them
	 * out. Any keys that have been converted will not appear in the resultant set.
	 * Any keys not converted will appear in the resultant set the same as the
	 * received set.
	 * <p>
	 * This will put the converted values into <code>to</code>, unless it is null in
	 * which case a temporary AttributeSet will be returned.
	 */
	...
	/**
	 * Buffer for the purpose of attribute conversion. This can be used if convAttr
	 * is being used.
	 */
	private MutableAttributeSet oConvAttr = new SimpleAttributeSet();

	/**
	 * Create an older style of HTML attributes. This will convert character level
	 * attributes that have a StyleConstants mapping over to an HTML tag/attribute.
	 * Other CSS attributes will be placed in an HTML style attribute.
	 */
	...
	/**
	 * Create/update an HTML &lt;font&gt; tag attribute. The value of the attribute
	 * should be a MutableAttributeSet so that the attributes can be updated as they
	 * are discovered.
	 */
	...
	/**
	 * Writes the line separator. This is overriden to make sure we don't replace
	 * the newline content in case it is outside normal ascii.
	 */
	...
	protected void output(char[] chars, int start, int length) throws IOException {
		...
		for (int counter = start; counter < length; counter++) {
			...
			default:
				 ...
				 * characters by numeric values by default. Use _nument=true if you need that
				 * substitution Original "if" condition: if (chars [counter]< ' ' || chars
				 * [counter] > 127)
				 */
	...
	/**
	 * This class extends DefaultListModel, and also implements the
	 * ListSelectionModel interface, allowing for it to store state relevant to a
	 * SELECT form element which is implemented as a List. If SELECT has a size
	 * attribute whose value is greater than 1, or if allows multiple selection then
	 * a JList is used to represent it and the OptionListModel is used as its model.
	 * It also stores the initial state of the JList, to ensure an accurate reset,
	 * if the user requests a reset of the form.
	 *
	 * @author Sunita Mani
	...
	class OptionListModel extends DefaultListModel implements ListSelectionModel, Serializable {
		...
		/**
		 * Returns an array of all the <code>ListSelectionListener</code>s added to this
		 * OptionListModel with addListSelectionListener().
		 *
		...
		/**
		 * Notify ListSelectionListeners that the value of the selection, in the closed
		...
		private void clear(int r) {
			 ...
			 * is called the work of the previous deselection will not be repeated.
			 * 
			 * We also don't need to worry about the case when the min and max values are in
			 * their unassigned states. This cannot happen because this method's initial
			 * check ensures that the selection was not empty and therefore that the
			 * minIndex and maxIndex had 'real' values.
			 * 
			 * If we have cleared the whole selection, set the minIndex and maxIndex to
			 * their cannonical values so that the next set command always works just by
		 ...
		 * with bounds that cover all the changes to the selection plus the changes to
		 * the lead and anchor indices. Setting the flag to false causes a norrowing of
		 * the event's bounds to include only the elements that have been selected or
		 ...
		 * itself selected, set all of the newly inserted items, otherwise leave them
		 * unselected. This method is typically called to sync the selection model with
		 ...
		 * corresponding change in the data model. Note that (as always) index0 can be
		 * greater than index1.
		 */
		...
		/**
		 * Set the lead selection index, ensuring that values between the anchor and the
		 * new lead are either all selected or all deselected. If the value at the
		 ...
		 * been first cleared and only to later be set.
		 * <p>
		 * This method can be used in the mouseDragged() method of a UI class to extend
		 * a selection.
		 *
		...
		/**
		 * This method is responsible for storing the state of the initial selection. If
		 * the selectionMode is the default, i.e allowing only for SINGLE_SELECTION,
		 * then the very last OPTION that has the selected attribute set wins.
		 */
		...
		/**
		 * Fetches the BitSet that represents the initial set of selected items in the
		 * list.
		 */
	...
	class OptionComboBoxModel extends DefaultComboBoxModel implements Serializable {
		...
		/**
		 * Fetches the Option item that represents that was initially set to a selected
		 * state.
		 */
	...
	/**
	 * Map is used to represent a map element that is part of an HTML document. Once
	 * a Map has been created, and any number of areas have been added, you can test
	 * if a point falls inside the map via the contains method.
	 *
	 * @author Scott Violet
	...
	class Map {
		...
		/**
		 * An array of RegionContainments, will slowly grow to match the length of
		 * areaAttributes as needed.
		 */
		 ...
		 * <code>x</code>, <code>y</code>. <code>width</code>, <code>height</code> gives
		 * the size of the region the map is defined over. If a matching area is found,
		 * the AttribueSet for it is returned.
		 */
		...
		/**
		 * Creates and returns an instance of RegionContainment that can be used to test
		 * if a particular point lies inside a region.
		 */
	...
	/**
	 * An implementation that will return true if the x, y location is inside a
	 * rectangle defined by origin 0, 0, and width equal to width passed in, and
	 * height equal to height passed in.
	 */
	...
	static class PolygonRegionContainment extends Polygon implements RegionContainment {
		 ...
		 * Use percentIndex to find out the index for it.
		 */
		float[] percentValues;
		int[] percentIndexs;
		/** Last value of width passed in. */
	...
	static class RectangleRegionContainment implements RegionContainment {
		/**
		 * Will be non-null if one of the values is a percent, and any value that is non
		 * null indicates it is a percent (order is x, y, width, height).
		 */
		float[] percents;
		/** Last value of width passed in. */
		int lastWidth;
	 ...
	 * <code>stringCoords</code>. If one of the values represents a % the returned
	 * value with be negative. If a parse error results from trying to parse one of
	 * the numbers null is returned.
	 */
	...
	interface RegionContainment {
		/**
		 * Returns true if the location <code>x</code>, <code>y</code> falls inside the
		 * region defined in the receiver. <code>width</code>, <code>height</code> is
		 * the size of the enclosing region.
		 */
		public boolean contains(int x, int y, int width, int height);
	}
