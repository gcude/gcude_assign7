...
public class ResourcesListImpl implements IResourcesList {
	 ...
	 */
	/*
	 * public void addResource(String path, String taskId) { Element el = new
	 * Element("resource"); el.addAttribute(new Attribute("id", Util.generateId()));
	 * el.addAttribute(new Attribute("path", path)); if (taskId != null)
	 * el.addAttribute(new Attribute("taskId", taskId)); _root.appendChild(el); }
	 */

	/**
	 * @see main.java.memoranda.interfaces.IResourcesList#addResource(java.lang.String,
	 *      boolean)
	 */
	 ...
	 */
	/*
	 * public Vector getResourcesForTask(String taskId) { Vector v = new Vector();
	 * Elements rs = _root.getChildElements("resource"); for (int i = 0; i <
	 * rs.size(); i++) if
	 * (rs.get(i).getAttribute("taskId").getValue().equals(taskId))
	 * v.add(rs.get(i).getAttribute("path").getValue()); return v; }
	 */
